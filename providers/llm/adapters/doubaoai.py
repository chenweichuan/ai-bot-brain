...
class DoubaoaiLlmAdapter(LlmClient):
    ...
    async def chat(self, **request):
        ...
        is_openai_format = "messages" in request

        # OpenAI 格式转换为 Ark Responses API 请求格式
        if is_openai_format:
            request = await self._convert_to_ark_request(request)
        ...
        if request.get("stream") == True:
                        ...
                        async with client.stream(
                            ...
                            async for line in response.aiter_lines():
                                ...
                                try:
                                    ...
                                    if event.get("usage"):
                                        logger.info(f"[DoubaoAI] Token usage ({model_name}): {json.dumps(event['usage'], ensure_ascii=False)}")
                                    ...
                                    if event_type == "response.created":
                                        ...
                                        chunk = self._convert_to_openai_initial_chunk(
                                        ...
                                        if item_type == "function_call":
                                            ...
                                            chunk = self._convert_to_openai_tool_call_start_chunk(
                                        ...
                                        if delta_reasoning:
                                            chunk = self._convert_to_openai_reasoning_delta_chunk(
                                        ...
                                        if tc_index is not None and delta_args:
                                            chunk = self._convert_to_openai_tool_call_delta_chunk(
                                        ...
                                        if resp.get("model"):
                                            model_name = resp["model"]
                                        # 转换 usage 格式
                                        usage = self._convert_to_openai_usage(usage)
                                        ...
                                        chunk = self._convert_to_openai_final_chunk(
                                            response_id, model_name, status, usage, system_fingerprint,
                                        ...
                                        usage_inc = self._convert_to_openai_usage(usage_inc) if usage_inc else {}
                                        chunk = self._convert_to_openai_final_chunk(
                                            response_id, model_name, "length", usage_inc, system_fingerprint
                                        )
                                        yield chunk
                                        ...
                                        if delta_text:
                                            chunk = self._convert_to_openai_text_delta_chunk(
            ...
            async with httpx.AsyncClient() as client:
                try:
                    ...
                    # 转换为 OpenAI 兼容的响应格式
                    if is_openai_format:
                        result = self._convert_to_openai_response(result, request["model"])
    ...
    async def _convert_to_ark_request(self, request: dict) -> dict:
    ...
    def _convert_to_openai_response(self, ark_response: dict, model_name: str) -> dict:
        ...
        usage = self._convert_to_openai_usage(ark_usage)
    ...
    def _convert_to_openai_usage(self, ark_usage: dict) -> dict:
    ...
    def _convert_to_openai_initial_chunk(self, response_id: str, model_name: str, system_fingerprint: str = None) -> dict:
    ...
    def _convert_to_openai_text_delta_chunk(self, response_id: str, model_name: str, delta_text: str, system_fingerprint: str = None) -> dict:
        """构建文本增量流式 chunk"""
        chunk = {
            "id": response_id or "",
            "object": "chat.completion.chunk",
    ...
    def _convert_to_openai_reasoning_delta_chunk(self, response_id: str, model_name: str, delta_reasoning: str, system_fingerprint: str = None) -> dict:
        """构建推理内容增量流式 chunk"""
        chunk = {
            "id": response_id or "",
            "object": "chat.completion.chunk",
    ...
    def _convert_to_openai_tool_call_start_chunk(self, response_id: str, model_name: str, tool_call: dict, system_fingerprint: str = None) -> dict:
        """构建工具调用开始的流式 chunk"""
        chunk = {
            "id": response_id or "",
            "object": "chat.completion.chunk",
    ...
    def _convert_to_openai_tool_call_delta_chunk(self, response_id: str, model_name: str, tc_index: int, delta_args: str, system_fingerprint: str = None) -> dict:
        """构建工具调用参数增量流式 chunk"""
        chunk = {
            "id": response_id or "",
            "object": "chat.completion.chunk",
    ...
    def _convert_to_openai_final_chunk(self, response_id: str, model_name: str, status: str, usage: dict, system_fingerprint: str = None, has_tool_calls: bool = False) -> dict:
