...
async def get_base64_data_url(url: str, media_type: str) -> str:
    """
    ...
    path = await get_cached_tmp_file(url, media_type)
...
async def get_cached_tmp_file(source: str, media_type: str = None) -> str:
    """
    ...
    cache_key = f"{_TMP_FILE_CACHE_PREFIX}{unique_key}"
    ...
    # 压缩处理
    if media_type == "image":
        await compress_image(tmp_path)
    elif media_type == "video":
        await compress_video(tmp_path)

    # 写入Redis缓存，设置7天过期
    await redis.set(cache_key, tmp_path, ex=_TMP_FILE_CACHE_TTL)

    return tmp_path

async def compress_image(input_path: str) -> None:
    """
...
async def compress_video(input_path: str) -> None:
    """
