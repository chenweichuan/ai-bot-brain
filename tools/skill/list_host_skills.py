...
class ListHostSkillsTool(Tool):
    """Host skill list tool - allows agent to list all available host skills"""
    
    name = "list_host_skills"
    ...
    async def get_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "function": {
                "name": self.name,
                "description": f"Refresh and list all available host skills under {self.skill_dir} with descriptions. "
                    "Use this to get the latest host skill list or learn what each skill does before loading one.",
                "parameters": {
                    "type": "object",
                    "properties": {},
                    "required": [],
    ...
    async def execute(self, arguments: str) -> tuple[str, str]:
        """Execute skill list"""
        try:
            # List all available host skills
            ...
            logger.error(f"[ListHostSkillsTool] Failed to list skills: {e}")
