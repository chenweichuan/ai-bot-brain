...
class DeleteHostFileLinksTool(Tool):
    """Delete host file links tool - delete short links and remove stored files"""
    
    name = "delete_host_file_links"
    ...
    async def get_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "function": {
                "name": self.name,
                "description": "Delete host short links and their associated stored files. "
    ...
    async def execute(self, arguments: str) -> tuple[str, str]:
        ...
        for link in links:
            try:
                ...
                logger.error(f"[DeleteHostFileLinks] Error processing {link}: {str(e)}")
                results.append({
                    "link": link,
                    "status": "failed",
                    "error": str(e)
