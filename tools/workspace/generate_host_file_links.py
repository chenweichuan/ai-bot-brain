...
class GenerateHostFileLinksTool(Tool):
    """Generate host file links tool - copy files from workspace to storage and create short links in bulk"""
    
    name = "generate_host_file_links"
    ...
    async def get_definition(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "function": {
                "name": self.name,
                "description": f"Generate host links for multiple files for easy sharing. "
    ...
    async def execute(self, arguments: str) -> tuple[str, str]:
        ...
        for file_path in file_paths:
            try:
                ...
                logger.error(f"[GenerateHostFileLinks] Error processing {file_path}: {str(e)}")
                results.append({
                    "file_path": file_path,
                    "status": "failed",
                    "error": str(e)
