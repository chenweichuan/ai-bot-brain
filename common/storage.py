...
class Storage():
    ...
    async def save(file) -> str:
        ...
        if isinstance(file, str) and file.startswith(Storage.base_url):
            return Storage.url_to_path(file)
        elif isinstance(file, str) and file.startswith(Storage.base_path):
            return file
        ...
        if original_filename:
            ...
            dir_path = f"{Storage.base_path}/{file_md5}"
            ...
            if file_kind:
                ...
                try:
                    ...
                    file_ext = "bin"
            file_path = f"{Storage.base_path}/{file_md5}.{file_ext}"
        ...
        # 异步审核文件
        asyncio.create_task(Storage._moderate_file_background(file_path))
    ...
    def path_to_url(path: str) -> str:
        ...
        return Storage.base_url + path.replace(Storage.base_path, '')

    @staticmethod
    def url_to_path(url: str) -> str:
        """Convert storage URL to path"""
        return Storage.base_path + url.replace(Storage.base_url, '')
    ...
    async def _moderate_file_background(file_path: str):
        ...
        Args:
            file_path: 文件的本地路径
        """
        file_url = Storage.path_to_url(file_path)
            ...
            if result.is_block():
                ...
                try:
                    if os.path.exists(file_path):
                        # 规范化路径，避免路径格式差异导致的问题
                        normalized_base_path = os.path.normpath(Storage.base_path)
